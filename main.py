...
class GameRunner:
    ...
    def loop(self):
        ...
        while True:
            ...
            if self.auto and not game_over:
                ...
                #print(_iter)
                _iterations.append(_iter)
                score_list.append(self.game.score)
                # if _iter == 100:
                #     print((_iterations, score_list))
                #     print()
                _iter += 1
    ...
    def get_scores(self):
        ...
        score_list = []
        _iterations_list = []
        self.game.tile_matrix = self.game.new_tile_matrix()
        self.game.place_random_tile()
        self.game.place_random_tile()

        while _iter < max_iterations:
            ai = AI(self.game.get_state())
            direction = ai.compute_decision()
            if direction != None:
                self.game.move_and_place(direction)
            score_list.append(self.game.score)
            _iterations_list.append(_iter)
            print(self.game.score)
            print(_iter)
            _iter += 1
            ...
            # self.print_matrix()
            # pygame.display.update()
        return (score_list, _iterations_list)

    def get_scores_improved(self):
        max_iterations = 2000
        _iter = 0
        score_list = []
        _iterations_list = []
...
if __name__ == '__main__':
    if args.test == 1:
        ...
        ROTATIONS = {pygame.K_UP: 0, pygame.K_DOWN: 2, pygame.K_LEFT: 1, pygame.K_RIGHT: 3}
        game = GameRunner()
        print(game.get_scores_improved())
    else:
        import pygame
        from pygame.locals import *
        ROTATIONS = {pygame.K_UP: 0, pygame.K_DOWN: 2, pygame.K_LEFT: 1, pygame.K_RIGHT: 3}
